...
async def updateAnalyticsTableV1(callingComponent,analyticsOrgId,analyticsServerUri,accessToken,ownerEmail,workspaceName,tableName,requestId,updateDict,dateFormat):
    configJson={}
    ...
    if dateFormat is not None:
        configJson["ZOHO_DATE_FORMAT"]=dateFormat
    ...
    try:
        ...
        apiResp=await loop.run_in_executor(None, lambda:requests.post(updateUrl,headers=apiHeader,data=configJson))
...
#V1 update API retry
async def updateAnalyticsApiV1WithRetry(callingComponent,analyticsOrgId,analyticsServerUri,accessToken,ownerEmail,workspaceName,tableName,requestId,updateDict,dateFormat):
    for _ in range(nRetries):
        ...
        apiResp=await updateAnalyticsTableV1(callingComponent,analyticsOrgId,analyticsServerUri,oauthToken,ownerEmail,workspaceName,tableName,requestId,updateDict,dateFormat)
...
async def asyncUpdateAnalyticsApiV1WithRetry(callingComponent,analyticsOrgId,analyticsServerUri,accessToken,ownerEmail,workspaceName,tableName,requestId,updateDict,dateFormat=None):
    task=asyncio.create_task(updateAnalyticsApiV1WithRetry(callingComponent,analyticsOrgId,analyticsServerUri,accessToken,ownerEmail,workspaceName,tableName,requestId,updateDict,dateFormat))
    return task
...
def updateAnalyticsTable(callingComponent,analyticsOrgId,analyticsServerUri,accessToken,workspaceId,viewId,requestId,updateDict,dateFormat):
    writeLog(callingComponent,infoLogLevel,"Inside updateAnalyticsTable")
    configJson={}
    ...
    try:
        # loop=asyncio.get_event_loop()
        #apiResp=await loop.run_in_executor(None,lambda:requests.put(updateUrl,headers=apiHeader))
        ...
        writeLog(callingComponent,errorLogLevel,"Error in Updation Call: "+traceback.format_exc())
    writeLog(callingComponent,infoLogLevel,"Analytics API response received: "+apiResp.text)
    try:
        writeLog(callingComponent,infoLogLevel,"Parsing API response received")
...
async def updateAnalyticsTableWithRetry(callingComponent,analyticsOrgId,analyticsServerUri,workspaceId,viewId,requestId,updateDict,dateFormat):
    for _ in range(nRetries):
        writeLog(callingComponent,infoLogLevel,"Getting access token.")
        accessToken=generateAccessToken.getAccessToken(zohoAccountsServerUri,clientId,clientSecret,code)
        writeLog(callingComponent,infoLogLevel,"Getting access token.")
        oauthToken=accessToken["accessToken"]
        writeLog(callingComponent,infoLogLevel,"Trying to update Analytics input table. Trial:{0}".format(_+1))
        apiResp=await updateAnalyticsTable(callingComponent,analyticsOrgId,analyticsServerUri,oauthToken,workspaceId,viewId,requestId,updateDict,dateFormat)
        writeLog(callingComponent,infoLogLevel,"Checking for status in update response")
...
async def asyncUpdateAnalyticsTableWithRetry(callingComponent,analyticsOrgId,analyticsServerUri,workspaceId,viewId,requestId,updateDict,dateFormat=None):
    task=asyncio.create_task(updateAnalyticsTableWithRetry(callingComponent,analyticsOrgId,analyticsServerUri,workspaceId,viewId,requestId,updateDict,dateFormat))
    return task
...
if __name__=="__main__":
    ...
    updateDict["jobCreatedDateTime"]=currentTime
    updateAnalyticsTable("Test",analyticsOrgId,analyticsServerUri,oauthToken,"100068000000029015","100068000003344940","1",updateDict,"dd/MM/yyyy HH:mm:ss")
    #updateAnalyticsTable("Test",analyticsOrgId,analyticsServerUri,oauthToken,"crmadmin1@unionbankofindia.bank","Desk_ccudesk","SFTP_export_requests","1",updateDict)